...
public class DataRouterManagerImplRequestingPlayerProfileTest {
    ...
    private final DataProvider playerProfiles = mock(DataProvider.class);
    ...
    public void providerFailureShouldResultInExceptionExplainingThat() throws DataProviderException {
        when(playerProfiles.getData(any(), any())).thenThrow(DataProviderException.class);
        Locale china = Locale.CHINA;
        Urn id = urnForAnyTournament();

        CommunicationException exception = catchThrowableOfType(
            () -> manager.requestPlayerProfileEndpoint(china, id, ANY_CACHE_ITEM),
            CommunicationException.class
        );

        assertEquals(
            "Error executing player profile request for id=" + id + ", locale=" + china,
    ...
    public void providerFailureShouldResultInExceptionIndicatingUrl() throws DataProviderException {
        ...
        when(playerProfiles.getData(any(), any())).thenThrow(dataProviderException);

        CommunicationException exception = catchThrowableOfType(
            () -> manager.requestPlayerProfileEndpoint(ANY_LANGUAGE, urnForAnyTournament(), ANY_CACHE_ITEM),
            CommunicationException.class
        );
