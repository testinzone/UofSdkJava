...
public class DataRouterManagerImplRequestingLotteryScheduleTest {
    ...
    private final DataProvider lotterySchedule = mock(DataProvider.class);
    ...
    public void providerFailureShouldResultInExceptionExplainingThat() throws DataProviderException {
        when(lotterySchedule.getData(any(), any())).thenThrow(DataProviderException.class);
        Locale china = Locale.CHINA;
        Urn id = urnForAnyTournament();

        CommunicationException exception = catchThrowableOfType(
            () -> manager.requestLotterySchedule(china, id, ANY_CACHE_ITEM),
            CommunicationException.class
        );

        assertEquals(
            "Error executing lottery schedule request for id=" + id + ", locale=" + china,
    ...
    public void providerFailureShouldResultInExceptionIndicatingUrl() throws DataProviderException {
        ...
        when(lotterySchedule.getData(any(), any())).thenThrow(dataProviderException);

        CommunicationException exception = catchThrowableOfType(
            () -> manager.requestLotterySchedule(ANY_LANGUAGE, urnForAnyTournament(), ANY_CACHE_ITEM),
            CommunicationException.class
        );
