...
public class ConfigurationBuilderTests extends ConfigurationBuilderSetup {
    ...
    @Test
    public void adjustAfterAgeIsSetTrue() {
        adjustAfterAgeCheck(true);
    }

    @Test
    public void adjustAfterAgeIsSetFalse() {
        adjustAfterAgeCheck(false);
    }

    private void adjustAfterAgeCheck(boolean adjustAfterAge) {
        UofConfiguration integrationConfig = buildConfig("i").setAdjustAfterAge(adjustAfterAge).build();
        UofConfiguration productionConfig = buildConfig("p").setAdjustAfterAge(adjustAfterAge).build();
        UofConfiguration replayConfig = buildConfig("r").setAdjustAfterAge(adjustAfterAge).build();
        UofConfiguration customConfig = buildCustomConfig().setAdjustAfterAge(adjustAfterAge).build();

        Assert.assertEquals(adjustAfterAge, integrationConfig.getProducer().adjustAfterAge());
        Assert.assertEquals(adjustAfterAge, productionConfig.getProducer().adjustAfterAge());
        Assert.assertEquals(adjustAfterAge, replayConfig.getProducer().adjustAfterAge());
        Assert.assertEquals(adjustAfterAge, customConfig.getProducer().adjustAfterAge());
    }
    ...
    public void statisticsIntervalCheck(int timeoutMinutes) {
        Duration timeout = Duration.ofMinutes(timeoutMinutes);
