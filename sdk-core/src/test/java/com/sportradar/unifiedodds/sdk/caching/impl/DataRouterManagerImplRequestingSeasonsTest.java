...
public class DataRouterManagerImplRequestingSeasonsTest {
    ...
    public void providerFailureShouldResultInExceptionExplainingThat() throws DataProviderException {
        when(seasons.getData(any(), any())).thenThrow(DataProviderException.class);
        Locale china = Locale.CHINA;
        Urn id = urnForAnyTournament();

        CommunicationException exception = catchThrowableOfType(
            () -> manager.requestSeasonsFor(china, id),
            CommunicationException.class
        );

        assertEquals(
            "Error executing tournament seasons request for id=" + id + ", locale=" + china,
    ...
    public void providerFailureShouldResultInExceptionIndicatingUrl() throws DataProviderException {
        ...
        when(seasons.getData(any(), any())).thenThrow(dataProviderException);

        CommunicationException exception = catchThrowableOfType(
            () -> manager.requestSeasonsFor(ANY_LANGUAGE, urnForAnyTournament()),
            CommunicationException.class
        );
