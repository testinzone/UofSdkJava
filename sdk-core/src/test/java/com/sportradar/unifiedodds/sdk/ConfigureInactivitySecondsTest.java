...
public class ConfigureInactivitySecondsTest {

    private final boolean replayMode = true;
    private final int amountInSeconds = 29;
    ...
    public void configureViaJavaApi() {
        UofConfiguration config = buildFromPropsFile
            ...
            .setInactivitySeconds(amountInSeconds)
            .build();
        ...
        assertEquals(asDuration(amountInSeconds), config.getProducer().getInactivitySeconds());
        assertEquals(amountInSeconds, internalConfig.getLongestInactivityInterval());
        assertEquals(amountInSeconds, internalConfigForReplay.getLongestInactivityInterval());
    }
