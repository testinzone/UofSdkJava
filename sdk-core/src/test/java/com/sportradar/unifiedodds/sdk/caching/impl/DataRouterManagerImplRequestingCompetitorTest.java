...
public class DataRouterManagerImplRequestingCompetitorTest {
    ...
    private final DataProvider competitors = mock(DataProvider.class);
    ...
    public void providerFailureShouldResultInExceptionExplainingThat() throws DataProviderException {
        when(competitors.getData(any(), any())).thenThrow(DataProviderException.class);
        Locale china = Locale.CHINA;
        Urn id = urnForAnyTournament();

        CommunicationException exception = catchThrowableOfType(
            () -> manager.requestCompetitorEndpoint(china, id, ANY_CACHE_ITEM),
            CommunicationException.class
        );

        assertEquals(
            "Error executing competitor profile request for id=" + id + ", locale=" + china,
    ...
    public void providerFailureShouldResultInExceptionIndicatingUrl() throws DataProviderException {
        ...
        when(competitors.getData(any(), any())).thenThrow(dataProviderException);

        CommunicationException exception = catchThrowableOfType(
            () -> manager.requestCompetitorEndpoint(ANY_LANGUAGE, urnForAnyTournament(), ANY_CACHE_ITEM),
            CommunicationException.class
        );
