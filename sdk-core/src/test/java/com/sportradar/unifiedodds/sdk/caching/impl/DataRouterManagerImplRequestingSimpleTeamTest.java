...
public class DataRouterManagerImplRequestingSimpleTeamTest {
    ...
    private final DataProvider simpleTeams = mock(DataProvider.class);
    ...
    public void providerFailureShouldResultInExceptionExplainingThat() throws DataProviderException {
        when(simpleTeams.getData(any(), any())).thenThrow(DataProviderException.class);
        Locale china = Locale.CHINA;
        Urn id = urnForAnyTournament();

        CommunicationException exception = catchThrowableOfType(
            () -> manager.requestSimpleTeamEndpoint(china, id, ANY_CACHE_ITEM),
            CommunicationException.class
        );

        assertEquals(
            "Error executing simpleteam profile request for id=" + id + ", locale=" + china,
    ...
    public void providerFailureShouldResultInExceptionIndicatingUrl() throws DataProviderException {
        ...
        when(simpleTeams.getData(any(), any())).thenThrow(dataProviderException);

        CommunicationException exception = catchThrowableOfType(
            () -> manager.requestSimpleTeamEndpoint(ANY_LANGUAGE, urnForAnyTournament(), ANY_CACHE_ITEM),
            CommunicationException.class
        );
